...
public interface ChatDAO {
    ...
    @Select("SELECT * FROM chat_log WHERE (`from` = LOWER(#{from}) AND `to` = LOWER(#{to}))  OR (`to` = LOWER(#{from}) AND `from` = LOWER(#{to})) ORDER BY `time` DESC LIMIT #{page}, 10")
    List<ChatDTO> getChatListByOpenid(@Param("from") String from, @Param("to") String to, @Param("page") Integer page);
}
